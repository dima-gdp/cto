...
export const setEventId = (event) => {
  Cookies.set(EVENT_KEY, event, { expires: cookieExpires || 1 })
}

export const getEventId = () => {
  const event = Cookies.get(EVENT_KEY)
  if (event) return event
  else return false
}
export const delEventId = () => {
  if (Cookies.get(EVENT_KEY)) {
    Cookies.remove(EVENT_KEY)
  }
}
...
export const setPermissions = (perm) => {
  if (perm) {
    Cookies.set(PERMISSIONS_KEY, perm, {expires: cookieExpires || 1})
  }
}
export const getPermissions = () => {
  const perm = Cookies.get(PERMISSIONS_KEY)
  if (perm) return JSON.parse(perm)
  else return []
}
export const delPermissions = () => {
  if (Cookies.get(PERMISSIONS_KEY)) {
...
export const getMenuByRouter = (list, access) => {
  ...
  forEach(list, (item) => {
    ...
    if (itemPermission && !permissions.includes(itemPermission)) {
      return
    }
...
const nanoid = customAlphabet('zxcvbnmasdfghjklqwertyuiop', 12)

export function getID() {
  // чтобы без цифр, а то html айди для использования в js нужно экранировать
  return nanoid()
}
...
export function fromStringToStamp(maskedString) {
  return luxon.DateTime.fromFormat(maskedString, 'dd-MM-yyyy HH:mm').toSeconds()
}

export function fromStampToString(stamp) {
  return luxon.DateTime.fromSeconds(+stamp).toFormat('dd-MM-yyyy HH:mm')
}

export function fromStampToStringWithoutTime(stamp) {
  return luxon.DateTime.fromSeconds(+stamp).toFormat('dd-MM-yyyy')
}

export function fromStringToISO(maskedString) {
  if (!maskedString) return null
  return luxon.DateTime.fromFormat(maskedString, 'dd-MM-yyyy HH:mm').toISO({
    suppressMilliseconds: true,
  })
}

export function fromISOToString(iso) {
  return luxon.DateTime.fromISO(iso).toFormat('dd-MM-yyyy HH:mm')
}
 ...
 * @returns {object}
 */
export function convertISOToStrDate(entity, fields) {
 ...
 * @returns {object}
 */
export function covertStrDateToISO(entity, fields) {
  return Object.fromEntries(
    Object.entries(entity).map(([key, value]) => {
 ...
 * @return {[]}
 */
export function makeArrayUnique(list) {
  return Array(...new Set([...list]))
}
...
function generateFormatFunction(formatName) {
  if (formatName === KEY_CASES_NAMES.CAMEL) {
    ...
    throw new Error(`Неизвестный формат ключей: ${formatName}`)
  }
}
