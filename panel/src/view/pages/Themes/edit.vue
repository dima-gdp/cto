  ...
  <div class="theme-create">
    <Row type="flex" justify="center" align="middle" class-name="view-header">
      <i-col class-name="view-header__divider">
        <Divider orientation="left">
          <span style="font-size: 36px">Редактирование темы</span>
        </Divider>
      </i-col>
    ...
    <Row type="flex" justify="end">
      <i-col :lg="3" :sm="6" class-name="org-form-actions">
        <Button class="org-form-actions__btn" long @click="updateTheme('accept')">
          Применить
        </Button>
      </i-col>
      <i-col :lg="3" :sm="6" class-name="org-form-actions" style="margin-left: 15px">
        <Button type="primary" class="org-form-actions__btn" long @click="updateTheme('save')">
          Сохранить
        </Button>
      </i-col>
    </Row>
...
<script>
import { updateTheme, getOneTheme } from '@/api/themes'
...
export default {
  ...
  async created() {
    ...
    this.loading = true
    const { data } = await getOneTheme(id).catch((e) => {
      this.loading = false
      throw new Error(e)
    })
    this.loading = false
    ...
    // Магия вне Хогвартса
    const root = {}
    for (const key in data.css[':root']) {
      root[`--${camelToKebab(key)}`] = data.css[':root'][key]
    }

    for (const key in this.theme.colors) {
      const category = this.theme.colors[key]

      category.forEach((color) => {
        if (root[color.name]) {
          color.value = root[color.name].replace(' !important', '')
        }
    ...
    for (const key in this.theme.fonts) {
      const font = this.theme.fonts[key]

      if (root[font.name]) {
        font.value = root[font.name].replace(' !important', '')
      }
    }
    ...
    async updateTheme(type) {
      ...
      if (valid) {
        ...
        await updateTheme(id, {
          ...
          this.loading = false
          this.$Message.error('Ошибка при обновлении темы')
          throw new Error(e)
        })
