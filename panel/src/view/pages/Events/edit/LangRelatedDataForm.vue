      ...
      <i-col span="24">
        <Card>
          ...
          <Form ref="contentInfo" :model="event" :rules="formRules" label-position="top">
            <Row type="flex" :gutter="15">
              <i-col :lg="16" :md="24" :sm="24">
                <FormItem class="content-info-form__item" label="Название мероприятия" prop="name">
                  <Input v-model="event.name" placeholder="Введите название мероприятия" />
                </FormItem>
              </i-col>
              ...
              <i-col :lg="5" :md="24" :sm="24">
                <FormItem class="content-info-form__item" label="Телефон" prop="phone">
                  <Input v-model="event.phone" placeholder="Введите телефон" />
                </FormItem>
              </i-col>

              <i-col :lg="5" :md="24" :sm="24">
                <FormItem class="content-info-form__item" label="Добавочный номер" prop="phoneInfo">
                  <Input v-model="event.phoneInfo" placeholder="Добавочный номер" />
                </FormItem>
              </i-col>
              ...
              <i-col :lg="12" :md="24" :sm="24">
                <FormItem
                  ...
                  prop="phoneText"
                >
                  <Input v-model="event.phoneText" placeholder="Введите текст под телефоном" />
                </FormItem>
              </i-col>

              <i-col :lg="12" :md="24" :sm="24">
                <FormItem class="content-info-form__item" label="Страница по умолчанию" prop="page">
                  <Select v-model="defaultPageId" filterable not-found-text="Ничего не найдено">
...
import { updateStaticPage } from '@/api/staticPages'
...
export default {
  ...
  data() {
    return {
      ...
      formRules: {
        ...
        phone: [{ required: true, message: 'Это поле обязательно!' }],
      },
  ...
  methods: {
    ...
    async saveDefaultPage() {
      const eventPages = this.event.pages
      if (eventPages && eventPages.length) {
        const { defaultPageId: selectedPageId } = this

        // сначала сбросить текущую страницу, иначе бек ругается
        const currentDefaultPage = this.event.pages.find((p) => p.type === 'default')
        if (currentDefaultPage) {
          await updateStaticPage(currentDefaultPage.id, { type: null }).catch((e) =>
            console.error(e)
          )
        }

        const pagesPromises = eventPages.map((p) => {
          updateStaticPage(p.id, { ...p, type: +p.id === +selectedPageId ? 'default' : null })
        })

        await Promise.all(pagesPromises).catch((e) => console.error(e))
      }
    },
    ...
    async getEvent(id, lang) {
      const params = { include: 'form-link,pages', lang }

      const { data: eventData } = await getEvent(id, params)
      this.event = eventData

      if (eventData.pages) {
        const defaultPage = eventData.pages.find((p) => p.type === 'default')
        this.defaultPageId = defaultPage && defaultPage.id
      }
